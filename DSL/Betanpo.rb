...
#                                  Fingered Pair                                       #
########################################################################################
# These tools are for montioring textual illussions generated by the monitor to watch  #
# how strongly it believes in specific hallucinations.                                 #
########################################################################################
define_method :monitor_illussion do |a1, b1, a2, b2, a3, b3|
  system("./floating_finger '#{a1}' '#{b1}'")
  system("./floating_finger '#{a2}' '#{b2}'")
  system("./floating_finger '#{a3}' '#{b3}'")
end

define_method :floating_finger do |a, b|
  right = [":right_finger", a.split(" ")]
  left  = [":left_finger",  b.split(" ")]

  floating = [":floating_finger", right[1][1], left[1][0]]

  puts "#{floating[0]} #{floating[1]} #{floating[2]}"
end
...
define_method :nesinon do |noun, non_color1, non_color2, valid_color|
  "#{noun} n'est #{non_color1} ou #{non_color2}, ou #{valid_color}. Quel est cette?"
end
...
define_method :negative_contrast do |not_cette, not_maisette, not_sinon|
  "#{not_cette} #{not_maisette} #{not_sinon}"
end
...
define_method :maisette do |base_number, first_number, second_number|
  system("./maisette #{base_number} #{first_number} #{base_number} #{second_number} #{base_number} #{first_number} #{second_number} >> ./graveyard/urn.txt; ./scatter_ashes")
end
...
define_method :track_fingers do | a1_a, b1_a, a2_a, b2_a, a3_a, b3_a,
                                  a1_b, b1_b, a2_b, b2_b, a3_b, b3_b,
                                  a1_c, b1_c, a2_c, b2_c, a3_c, b3_c |
                                  
  2.times do
    get_neg_statistics(:first_finger,  floating_finger(a1_a, b1_a),
                       :second_finger, floating_finger(a2_a, b2_a),
                       :third_finger,  floating_finger(a3_a, b3_a))
  ...
  2.times do
    get_neg_statistics(:first_finger,  floating_finger(a1_b, b1_b),
                       :second_finger, floating_finger(a2_b, b2_b),
                       :third_finger,  floating_finger(a3_b, b3_b))
  ...
  2.times do
    get_neg_statistics(:first_finger,  floating_finger(a1_c, b1_c),
                       :second_finger, floating_finger(a2_c, b2_c),
                       :third_finger,  floating_finger(a3_c, b3_c))
